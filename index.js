    ...
    bot.sendMessage(msg.chat.id, 'Digite seu peso, ' + msg.from.first_name,);
        ...
        vaiCalcular = false;
            ...
            if (calculo<18.5 && calculo>0.1){
                bot.sendMessage(msg.chat.id, "Você está magro com esse indice: " + calculo);
                }
                else if(calculo>=18.5 && calculo<24.9){
                bot.sendMessage(msg.chat.id, "Você está normal com esse indice: " + calculo);
                }
                else if(calculo>=25 && calculo<29.9) {
                bot.sendMessage(msg.chat.id,"Você está com sobre peso com esse indice: " + calculo);
                }
                else if(calculo>=30 && calculo<39.9) {
                bot.sendMessage(msg.chat.id,"Você está com obesidade com esse indice: " + calculo);
                }
                else if (calculo>40 && calculo <99999){
                bot.sendMessage(msg.chat.id,"Você está com obesidade grave com esse indice: " + calculo);
                }
                else if (calculo = isNaN || calculo<0.1)
                bot.sendMessage(msg.chat.id, 'Digite números, não temos como calcular sem números. \n Vamos tentar novamente, ' + msg.from.first_name,);
